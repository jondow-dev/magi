...
// Ported from Magi’s magimath.h or main.cpp
double exp_n(double x) {
    return std::exp(x);
}

double exp_n2(double x, double y) {
    return std::exp(x) * std::exp(y); // Placeholder; verify in magimath.h
}

// Magi’s GetDifficultyFromBits (from main.cpp)
double GetDifficultyFromBits(unsigned int nBits) {
    int nShift = (nBits >> 24) & 0xff;
    double dDiff = (double)0x0000ffff / (double)(nBits & 0x00ffffff);
    return dDiff * pow(2.0, 8.0 * (nShift - 3));
}
